...
(function (global, factory) {
  ...
  var IFrame = /*#__PURE__*/function () {
    ...
    // 刷新重置导航栏
    _proto.refresh = function(id) {
      // 导航宽度
      var navWidth = $__default["default"](SELECTOR_TAB_NAVBAR_NAV).width();
      // 所有菜单栏
      var navItem = $__default["default"](SELECTOR_TAB_NAVBAR_NAV).children('.nav-item');
      // 菜单宽度
      var itemWidth = 0;
      ...
      if (itemWidth > maxWidth) {
        ...
        navItem.each(function () {
          itemWidth +=$(this).outerWidth();
          if (itemWidth > maxWidth) {
            $(SELECTOR_TAB_NAVBAR_NAV).find('.more-tab .dropdown-menu').append($(this));
            navsum++;
          }
        });
        if (navsum>0) {
          $__default["default"](SELECTOR_TAB_NAVBAR_NAV).find('.more-tab').removeClass('d-none');
        }
      } else {
        // 没超出的情况下检测是否有下拉列表
        if ($(SELECTOR_TAB_NAVBAR_NAV).find('.more-tab .dropdown-menu').children().length>0) {
          var html = $(SELECTOR_TAB_NAVBAR_NAV).find('.more-tab .dropdown-menu').html();
          $__default["default"](SELECTOR_TAB_NAVBAR_NAV).append(html);
          $(SELECTOR_TAB_NAVBAR_NAV).find('.more-tab .dropdown-menu').html('');

          // 重新排列
          var navsum = 0;
          itemWidth = 0;
          $__default["default"](SELECTOR_TAB_NAVBAR_NAV).children('.nav-item').each(function () {
            itemWidth +=$(this).outerWidth();
            if (itemWidth > maxWidth) {
              $(SELECTOR_TAB_NAVBAR_NAV).find('.more-tab .dropdown-menu').append($(this));
              navsum++;
          ...
          if (navsum>0) {
            ...
            $__default["default"](SELECTOR_TAB_NAVBAR_NAV).find('.more-tab').addClass('d-none');
          }
        } else {
          $__default["default"](SELECTOR_TAB_NAVBAR_NAV).find('.more-tab').addClass('d-none');
        }
    ...
    _proto.createTab = function createTab(title, link, uniqueName, autoOpen) {
      ...
      // $(SELECTOR_TAB_NAVBAR_NAV).find('.more-tab .dropdown-menu').append(newNavItem);
    ...
    // 双击刷新
    _proto.switchTabReload = function switchTabReload(item, autoOpen) {
      ...
      if (!this._config.allowDuplicates && $__default["default"]("#" + navId).length > 0) {
        return this.switchTab("#" + navId, true);
      }
    };
    ...
    _proto.removeActiveTab = function removeActiveTab(type, element) {
      ...
      } else if (type == 'only-this') { // 关闭标签
        var $navClose = $__default["default"](element);
        var $navItem = $navClose.parent('.nav-item');
        var $navItemParent = $navItem.parent();
        var navItemIndex = $navItem.index();
        var tabId = $navClose.siblings('.nav-link').attr('aria-controls');
        // true 在下拉菜单，false 在导航栏
        var isDropdown = $navItemParent.is('.dropdown-menu');
        var isActive = $navItem.is('.active');

        // 删除标签与iframe
        $navItem.remove();
        $__default["default"]("#" + tabId).remove();
        if (isDropdown) {
          if ($navItemParent.children().length==0) {
            $__default["default"](SELECTOR_TAB_NAVBAR_NAV).find('.more-tab').addClass('d-none');
            if (isActive) {
              var prevNavItemIndex = $__default["default"](SELECTOR_TAB_NAVBAR_NAV).children('.nav-item').length - 1;
              this.switchTab($__default["default"](SELECTOR_TAB_NAVBAR_NAV).children('.nav-item').eq(prevNavItemIndex).find('a.nav-link'));
            }
          } else if (isActive) {
          ...
          if ($__default["default"](SELECTOR_TAB_CONTENT).children().length == $__default["default"](SELECTOR_TAB_EMPTY + ", " + SELECTOR_TAB_LOADING).length) {
            $__default["default"](SELECTOR_TAB_EMPTY).show();
          } else if (isActive) { // 只对删除激活标签页处理
            var length = $navItemParent.children('.nav-item').length;
            var prevNavItemIndex = navItemIndex > length ? navItemIndex - 2 : navItemIndex-1;
            this.switchTab($navItemParent.children('.nav-item').eq(prevNavItemIndex).find('a.nav-link'));
          }
          this.refresh();
        }
      } else {
    ...
    _proto._init = function _init() {
      ...
      // 判断是否初始化过
      var isInit = $__default["default"](SELECTOR_TAB_CONTENT).is('.isInit');
      if (!isInit) {
        $__default["default"](SELECTOR_TAB_CONTENT).addClass('isInit');
    ...
    _proto._setupListeners = function _setupListeners() {
      ...
      if ($__default["default"](SELECTOR_CONTENT_WRAPPER).hasClass(CLASS_NAME_IFRAME_MODE$1)) {
        ...
        // 双击刷新
        $__default["default"](document).on('dblclick', SELECTOR_SIDEBAR_MENU_ITEM + ", " + SELECTOR_SIDEBAR_SEARCH_ITEM, function (e) {
          e.preventDefault();
          e.stopPropagation()
          _this3.switchTabReload(e.target);
        });
    ...
    IFrame._jQueryInterface = function _jQueryInterface(config,title,link,uq,open) {
      if ($__default["default"](SELECTOR_DATA_TOGGLE$1).length > 0) {
        var data = $__default["default"](this).data(DATA_KEY$7);
        if (!data) {
          data = $.extend(true, {}, $__default["default"](this).data());
        }
        ...
        if (typeof config === 'string' && /createTab|openTabSidebar|switchTab|removeActiveTab/.test(config)) {
          plugin[config](title,link,uq,open);
        }
      } else {
  ...
  // edit:luo   $__default["default"](window).on('load', function () {
  $__default["default"](window).ready(function () {
    ...
    _proto.fixLayoutHeight = function fixLayoutHeight(extra) {
      ...
      if (offset !== false) {
        if (max === heights.controlSidebar) {
          ...
          $__default["default"](SELECTOR_CONTENT_IFRAME).height((max + offset - heights.header - heights.footer)- $__default["default"](SELECTOR_TAB_NAV).outerHeight());
        } else {
